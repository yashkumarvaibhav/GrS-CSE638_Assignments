/*
 * MT25091_Part_A3_Client.c
 * Zero-Copy TCP Client Implementation
 * Roll Number: MT25091
 *
...
void *client_thread(void *arg) {
  ...
  if (setsockopt(ctx->socket_fd, SOL_SOCKET, SO_ZEROCOPY, &one, sizeof(one)) <
      0) {
    perror("[Client] Warning: SO_ZEROCOPY not supported");
  }
  ...
  while (*(ctx->running) && get_time_sec() < end_time) {
    ...
    ssize_t bytes_sent = sendmsg(ctx->socket_fd, &send_hdr, MSG_ZEROCOPY);
...
int main(int argc, char *argv[]) {
  ...
  printf("==============================================\n");
  printf("  Zero-Copy TCP Client (MSG_ZEROCOPY) - MT25091\n");
  printf("==============================================\n");
  printf("Server: %s:%d\n", host, port);
